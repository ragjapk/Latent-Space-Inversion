...
class Generator(nn.Module):
    def __init__(self, latent_dim, num_domains, hidden_dim=512):
        super(Generator, self).__init__()
        self.model = nn.Sequential(
            nn.Linear(latent_dim + num_domains + num_domains, hidden_dim),
            nn.LeakyReLU(0.2),
...
class Discriminator(nn.Module):
    def __init__(self, latent_dim, num_domains, hidden_dim=512):
        ...
        self.domain_head = nn.Linear(hidden_dim // 2, num_domains)  # Domain classification
        self.real_fake_head = nn.Linear(hidden_dim // 2, 1)  # Real/Fake classification
    
    def forward(self, z):
        x = self.model(z)
        domain_pred = self.domain_head(x)  # No activation (use CrossEntropyLoss)
        real_fake_pred = self.real_fake_head(x)  # Use BCELoss
        return real_fake_pred, domain_pred
...
def load_stargan(ckpt='saved/stargan.pt', latent_dim = 256, hidden_dim=512, n_domains=3):
    g = Generator(latent_dim, n_domains, hidden_dim)
    g.load_state_dict(torch.load(ckpt))
    return g

if __name__=='__main__':
    g = Generator(256,5,1024)
    #g.load_state_dict(torch.load('stargan/models/80000-G.ckpt'))

    a = g(torch.randn([256,]), torch.randn([5,]),torch.randn([5,]))
    print(a.shape)
    d= Discriminator(256, 5, 1024).cuda()
    a,b = d(torch.randn([256,]).cuda())
